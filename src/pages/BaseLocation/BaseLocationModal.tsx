...
const BaseLocationModal: React.FC<BaseLocationModalProps> = ({ onClose, setBaseLocation, pickupLocation }) => {
    ...
    const formatDuration = (seconds: number | null): string => {
      ...
      const filteredItems = items
        .filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase()))
        .sort((a, b) => {
            const distanceA = distances[a.id]?.distance !== null ? parseFloat(distances[a.id]?.distance) : Infinity; // Convert distance to number
            const distanceB = distances[b.id]?.distance !== null ? parseFloat(distances[b.id]?.distance) : Infinity;
            return distanceA - distanceB; // Sort by distance
        });
